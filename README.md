...
The coremod patches Thaumcraft's `TileVisNode` lifecycle. After a newly loaded relay grid has had 40 ticks to register, every non-source relay performs one complete parent-chain check to confirm it reaches a live source node in a loaded chunk. A relay whose chain is broken reports the problem in the server log, loads and registers the nodes in its 3x3 chunk area, and tries to reconnect to a nearby healthy node while preserving range, attunement, and line-of-sight checks. The integrity check is deliberately not repeated for that loaded relay instance, so established relay layouts are never continuously rerouted.
...
```text
target/ThaumcraftVisRelayFix-1.1.7.jar
```
